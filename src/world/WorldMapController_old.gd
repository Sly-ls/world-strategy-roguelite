...
func _ready() -> void:
    ...
    var world_bounds = Rect2(0, 0, GRID_WIDTH * 64, GRID_HEIGHT * 64)
 ...
 # Zoom via actions
    ...
    if event is InputEventMouseButton:
        if event.button_index == MOUSE_BUTTON_WHEEL_UP:
            ...
            print("Zoom in: %.2f" % camera_controller.get_current_zoom())
        elif event.button_index == MOUSE_BUTTON_WHEEL_DOWN:
            camera_controller.adjust_zoom(-0.1)
            print("Zoom out: %.2f" % camera_controller.get_current_zoom())
 # Zoom via molette
